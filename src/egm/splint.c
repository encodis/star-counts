/*
  ...
  History  : Began modification: 6 October 1992. Last edit: 14 January 1993.
  ...
  Comments : 1. Original code from Numerical Recipes in C (Press, Flannery, 
	     ...
	     2. Modified by PJCH to  avoid use of  other NR  constructs and 
	        functions.
	     3. Function is now a doublefunction -  it returns  the value of
	        y directly rather than through an address. This fits better
		with the way it is used in  init_color() and  init_lumfn().
*/
...
double splint(double *xa, double *ya, double *y2a, int n, double x)
{
  ...
  h = xa[khi] - xa[klo];
  if (h == 0.0) {
    quit_egm(ERR_MATH,"Bad XA input","splint()");
  }
