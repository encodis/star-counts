/*
  ...
  Purpose  : MU space. Bahcall - Soneira Galaxy Model.
	     ...
	     being used.
  History  : Began: 25 August 1992. Last edit: 4 August 1994.
...
void writ_hed(Optar *op, Param *pm, Model *ml, Numbr *nm, FILE *out)
{
  ...
  fprintf(out,"# B & S GALAXY MODEL: %-36s    Components: %d \n", 
	  pfile, numcp);
  fprintf(out,"#     l = %9.3f     b = %9.3f     A = %9.3f   mag = %9.3f\n", 
	  ml->l2, ml->b2, ml->omega, pm->mag);
  fprintf(out,"# mubrt = %9.3f mudim = %9.3f delmu = %9.3f  Mbin = %9.3f\n",
	  pm->mu_brt, pm->mu_dim, pm->del_mu, pm->dm_bin);
  ...
  fprintf(out,"#    r0 = %9.3f   abs = %9.3f amode = %9s\n",
	  pm->r0, pm->abs, amode);
