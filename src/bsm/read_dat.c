/*
  ...
  History  : Began: 28 June 1994. Last edit: 30 June 1994.
  Author   : P.J.C.Hodder (hodder@geop.ubc.ca)
  Comments : 1. Function looks in CWD (.) for the file, then in the library.
             2. Reads two column data, ignores comments (#).
*/

#include "bsm.h"

void read_dat(char *file, char *libry, int *npt, REAL **x, REAL **y)
{
  FILE *inp;
  ...
  if ((inp = fopen(file,"r")) == NULL) {
    ...
    if ((inp = fopen(xfile,"r")) == NULL) {
      quit_bsm(ERR_OPEN,file,"read_dat()");
    }
  }
